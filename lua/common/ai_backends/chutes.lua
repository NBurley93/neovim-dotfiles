...
function M.get_url(_)
  return 'https://llm.ai/v1/chat/completions'
end

function M.get_headers()
  local chutes_token = vim.env.CHUTES_API_TOKEN
  if not chutes_token or chutes_token == '' then
    vim.notify('CHUTES_API_TOKEN is not set. Please set it to use the Chutes AI provider.', vim.log.levels.ERROR)
    return {}
  end
  return {
